...
Foreach ($item in $ProfileList) {
    ...
    get-childitem -Path "registry::HKEY_USERS\$($Item.SID)\Software\Policies\" | Where-Object{$_.name -like "*Microsoft*"} | remove-item -Recurse -force -Verbose -ErrorAction SilentlyContinue
    get-childitem -Path "registry::HKEY_USERS\$($Item.SID)\Software\Microsoft\Windows\CurrentVersion\" | Where-Object{$_.name -like "*Group Policy Objects*"} | remove-item -Recurse -force -Verbose -ErrorAction SilentlyContinue
    get-childitem -Path "registry::HKEY_USERS\$($Item.SID)\Software\Microsoft\Windows\CurrentVersion\" | Where-Object{$_.name -like "*Policies*"} | remove-item -Recurse -force -Verbose -ErrorAction SilentlyContinue
