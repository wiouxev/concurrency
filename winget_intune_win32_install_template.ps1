...
***Make sure ALL locations within script calling to the .ps1 file name MATCH!!***
#>
...
If ($ENV:PROCESSOR_ARCHITEW6432 -eq "AMD64")
{
	Try
	{
		...
		Start-Process -FilePath "$ENV:WINDIR\SysNative\WindowsPowershell\v1.0\PowerShell.exe" -ArgumentList "-File `"$($PSScriptRoot)\winget_intune_win32_install_template.ps1`" $($argsString)" -Wait -NoNewWindow
	}
	Catch
	{
